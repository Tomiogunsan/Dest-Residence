...
export default function Header() {
    const [pageState, setPageState] = useStat('Sign in')
        ...
        <header className=' flex justify-between items-center px-3 py-2 max-w-[1270px] mx-auto'>
            ...
            <div>
                <ul className='flex '>
                    ...
                    <li className={ `cursor-pointer py-3 text-sm font-semibold text-gray-400 border-b-[3px] border-b-transparent 
                    ${(pathMatchRoute('/sign-in')  || pathMatchRoute('/profile'))&& 'text-black border-b-blue-600'}`} onClick={()=> navigate('/profile')}>Sign in</li>
                </ul>
            </div>
        </header>
    </div>
